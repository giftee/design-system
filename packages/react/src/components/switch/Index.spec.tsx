...
describe('Switch', () => {
  ...
  test('clickイベントが正しく処理される', async () => {
    ...
    const { getByRole } = render(
      <Switch name="test" onChange={handleChange}>クリック可能スイッチ</Switch>
    );
  ...
  test('disabledプロパティが正しく動作する', () => {
    const { getByRole } = render(
      <Switch name="test" disabled>無効化スイッチ</Switch>
    );
  ...
  test('classNameプロパティが追加で指定できる', () => {
    const { container } = render(
      <Switch name="test" className="custom-class">カスタムクラススイッチ</Switch>
    );
